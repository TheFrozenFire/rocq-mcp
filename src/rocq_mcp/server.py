...
def _workspace_has_project_marker(ws: Path) -> bool:
    ...
    - dune ancestor match: ``_find_dune_root`` walks UP from *ws* looking
      ...
      walk via ``dune {coq,rocq} top``.  This prevents spurious warnings on a
...
# ---------------------------------------------------------------------------
# Per-workspace ".vo epoch" — read-side companion to _maybe_vo_rebuild_warning.
# A held state_id freezes the Coq environment at creation, including any
# dependency .vo already loaded by Fleche; a later rebuild of that .vo does not
# invalidate it, so ``proof_finished`` can diverge from a clean compile.  We
# bump a monotonic per-workspace counter whenever a compile through this server
# rewrites a .vo, stamp each state with the epoch at session start, and flag
# staleness when the epoch has since advanced (see interactive._check_staleness).
# Catches every rebuild done through this server; an external rewrite (a
# terminal ``dune build``) is not observed.
# ---------------------------------------------------------------------------


def _current_vo_epoch(lifespan_state: dict[str, Any], workspace: str) -> int:
    """Return the current .vo epoch for *workspace* (0 if never rebuilt)."""
    try:
        key = str(Path(workspace).resolve())
    except OSError:
        key = workspace
    return lifespan_state.get("vo_epochs", {}).get(key, 0)


def _bump_vo_epoch_if_rebuilt(
    lifespan_state: dict[str, Any],
    workspace: str,
    before_mtimes: dict[str, float] | None,
    after_mtimes: dict[str, float] | None,
) -> None:
    """Increment *workspace*'s .vo epoch when a compile rewrote any .vo.

    No-op when either snapshot is ``None`` (workspace unscanned / too large)
    or nothing changed.  Keyed by the resolved workspace path so it matches
    :func:`_current_vo_epoch`.
    """
    if before_mtimes is None or after_mtimes is None:
        return
    if not _diff_vo_mtimes(before_mtimes, after_mtimes):
        return
    try:
        key = str(Path(workspace).resolve())
    except OSError:
        key = workspace
    epochs = lifespan_state.setdefault("vo_epochs", {})
...
# Matches both the legacy ``(coq.theory ...)`` and the modern
# ``(rocq.theory ...)`` dune stanzas.  The latter is emitted by dune
# projects declaring ``(using rocq N)`` and pairs with ``dune rocq top``
# (see :func:`_run_dune_coq_top`).
_COQ_THEORY_RE = re.compile(r"^\s*\((?:rocq|coq)\.theory\b", re.MULTILINE)
...
def _pick_v_file(directory: Path) -> Path | None:
    ...
    ``dune {coq,rocq} top`` confuses dune.  Shallow ``*.v`` covers the common
...
def _find_coq_theory_dirs(ws: Path) -> list[Path]:
    ...
    ``(coq.theory ...)`` or ``(rocq.theory ...)`` stanza.

    Anchored regex: the stanza must begin a line (optionally indented)
    with ``(coq.theory`` / ``(rocq.theory`` followed by a word boundary.
    This avoids false positives in line comments (``; (coq.theory ...)``)
    while matching every well-formed top-level stanza.  Used only to
    *enumerate* theory roots so we know how many ``dune {coq,rocq} top``
    calls to make; the returned flags themselves still come from
    ``dune {coq,rocq} top`` (the source of truth for paths and flags).
    """
    dirs: list[Path] = []
...
# Dune toplevel subcommands, in the order we try them.  The legacy Coq
# plugin — ``(coq.theory ...)`` stanzas — answers ``dune coq top``; the
# modern Rocq plugin — ``(using rocq N)`` / ``(rocq.theory ...)`` — answers
# ``dune rocq top`` (``dune coq top`` fails there with "Cannot find file").
# A project uses one flavor or the other, and the wrong subcommand fails
# fast (nonzero exit), so falling through to the next is cheap.  Legacy
# first preserves historical behavior for existing coq-stanza projects.
#
# Horizon: dune deprecates the Coq Build Language and plans to remove
# ``dune coq top`` in dune 3.24.  Once the supported floor moves past that,
# the ``coq`` entry becomes dead weight (a wasted exit-1 spawn per theory)
# and the ordering can be flipped to ``rocq`` first (or ``coq`` dropped).
_DUNE_TOPLEVEL_SUBCOMMANDS: tuple[tuple[str, ...], ...] = (
    ("dune", "coq", "top"),
    ("dune", "rocq", "top"),
)
...
def _run_dune_coq_top(
    ...
    """Run ``dune {coq,rocq} top --toplevel echo --no-build <v_rel>`` from
    *dune_root* and return the parsed shell args, or ``None`` on
    subprocess failure / nonzero exit / parse error.

    Tries the legacy ``dune coq top`` first, then the modern ``dune rocq
    top`` (dune ``(using rocq ...)`` projects, whose ``(rocq.theory ...)``
    stanzas ``dune coq top`` cannot resolve).  A missing ``dune`` binary or
    a timeout aborts immediately — retrying the other subcommand would hit
    the same wall.
    """
    import shlex

    for subcommand in _DUNE_TOPLEVEL_SUBCOMMANDS:
        try:
            result = subprocess.run(
                [*subcommand, "--toplevel", "echo", "--no-build", v_rel],
                capture_output=True,
                text=True,
                timeout=timeout,
                cwd=str(dune_root),
...
def _dune_path_to_ws_relative(dir_arg: str, ws: Path, dune_root: Path) -> str | None:
    """Validate a path from ``dune {coq,rocq} top`` output and make it relative to *ws*.
...
def _select_representative_v_files(ws: Path) -> list[Path]:
    """Pick one ``.v`` per ``(coq.theory ...)`` / ``(rocq.theory ...)`` directory under *ws*.
...
def _parse_dune_args(
    ...
    """Parse a flat list of ``dune {coq,rocq} top`` args, deduping by semantic key.
    ...
    ``(-noinit,)``.  Required because running ``dune {coq,rocq} top`` against
...
def _parse_dune_flags(ws: Path) -> list[str] | None:
    """Extract coqc flags from a dune project via ``dune {coq,rocq} top``.

    If a ``dune-project`` file exists in *ws* (or a parent), discovers
    every ``(coq.theory ...)`` / ``(rocq.theory ...)`` directory under
    *ws*, runs ``dune {coq,rocq} top --toplevel echo --no-build <file.v>``
    once per theory (using a representative ``.v`` from each; see
    :func:`_run_dune_coq_top` for the legacy-vs-modern subcommand
    fallback), and unions the resulting flags (deduplicated).  This is
    required for dune workspaces with multiple theories: querying a single
    theory yields flags for that theory only, leaving cross-theory imports
    silently broken.
    ...
    # Run dune {coq,rocq} top once per representative file and union the args.
    all_args: list[str] = []
...
def _parse_project_flags(ws: Path) -> list[str]:
    ...
    ``dune {coq,rocq} top``.  If that also fails, returns
